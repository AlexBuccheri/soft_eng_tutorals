...
def bukin_n6(point: np.ndarray) -> float:
    ...
    if point.size != 2:
        raise ValueError("Bukin N.6 function is only defined for 2-dimensional input.")
    x, y = point
...
def eggholder(point: np.ndarray) -> float:
    ...
    point = np.asarray(point)
    if point.size != 2:
        raise ValueError("Eggholder function is only defined for 2-dimensional input.")
    x, y = point
...
def powell(x: np.ndarray) -> float:
    ...
    x = np.asarray(x)
    if x.size != 4:
        raise ValueError("Powell function is only defined for 4-dimensional input.")
    term1 = (x[0] + 10 * x[1]) ** 2
    term2 = 5 * (x[2] - x[3]) ** 2
    term3 = (x[1] - 2 * x[2]) ** 4
    term4 = 10 * (x[0] - x[3]) ** 4
...
def derivative_rosenbrock(x: np.ndarray) -> np.ndarray:
    ...
    df[-1] = 200 * (x[-1] - x[-2]**2)
    df[1:-1] = -400 * x[1:-1] * (x[2:] - x[1:-1]**2) - 2 * (1 - x[1:-1]) + 200 * (x[1:-1] - x[:-2]**2)
    return df
...
def six_hump_camel(point: np.ndarray) -> float:
    ...
    point = np.asarray(point)
    if point.size != 2:
        raise ValueError("Six-Hump Camel function is only defined for 2-dimensional input.")
    x, y = point
