...
def line_search_strong_wolfe_conditions(f: FuncType,
                          df: FuncType,
                          x: np.ndarray,
                          d: np.ndarray,
                          alpha0: float,
    ...
    assert 0 < c1 < c2, \
        "Decrease parameter, c1, must be > 0 and < c2"

    assert c1 < c2 < 1, \
        "Second decrease parameter, c2, must be > c1 and < 1"
    ...
    while f(x_increment) > f_x + (c1 * alpha * g_dot_d) and \
        np.abs(np.dot(df(x_increment), d)) > c2 * np.abs(g_dot_d):
        alpha *= reduction_factor
        x_increment = x + alpha * d
