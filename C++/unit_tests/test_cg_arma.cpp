...
TEST_CASE("Non-Linear Conjugate Gradient", "[optimiser]") {
    ...
    LineSearchFunc line_search_func =
        ...
        return line_search_backtrack(f, df, x, direction, alpha0, my_reduction, my_c1);
    };

    const auto cg_result = cg::nonlinear::non_linear_conjugate_gradient(f, df, x0, line_search_func, 2000, 1.e-5);
