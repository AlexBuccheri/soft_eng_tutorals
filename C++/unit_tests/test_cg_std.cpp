...
TEST_CASE("Vector utility operations", "[algorithms]") {
    ...
    SECTION( "Dot product of two vectors" ) {
        const double u_dot_v = algorithms::stl::dot(u, v);
        REQUIRE_THAT( u_dot_v, WithinAbs(3, 1.e-10) );
    }

    SECTION( "Norm of a vector" ) {
        const double norm_u = algorithms::stl::norm(u);
    ...
    SECTION( "Subtract vector v from vector u" ) {
        ...
        const auto u_mins_v = algorithms::stl::vector_subtract(u, v);
    ...
    SECTION( "Add vector v to vector u" ) {
        ...
        const auto u_plus_v = algorithms::stl::vector_add(u, v);
    ...
    SECTION( "Scale a vector by a constant" ) {
        ...
        const auto scaled_u = algorithms::stl::vector_scale(u, -1.0);
        REQUIRE(scaled_u.size() == 4);
        REQUIRE(scaled_u == ref_scaled_u);
    }

    SECTION( "Matrix-vector Product" ) {
        const algorithms::stl::matrix a {{1.0,  3.0, 4.0},
                                         {6.0,  1.0, 5.0},
                                         {2.0, 10.0, 1.0}};

        const std::vector<double> x {1.0, 2.0, 3.0};

        const auto product = algorithms::stl::matrix_vector_product(a, x);
    ...
    SECTION( "Matrix-vector Product Inconsistent Dimensions" ) {
        ...
        REQUIRE_THROWS_AS(algorithms::stl::matrix_vector_product(a, x), std::invalid_argument);
        REQUIRE_THROWS_WITH(algorithms::stl::matrix_vector_product(a, x),
                            ContainsSubstring( "Vector x must have length equal to" ));
    }
}


TEST_CASE("Linear Conjugate Gradient implemented with std::vector", "[algorithms]") {
    // SPD tri-diagonal matrix
    const algorithms::stl::matrix a {
            {4, 1, 0, 0, 0},
            {1, 4, 1, 0, 0},
            {0, 1, 4, 1, 0},
            {0, 0, 1, 4, 1},
    ...
    const auto x = algorithms::stl::linear_conjugate_gradient(a, x0, b);
